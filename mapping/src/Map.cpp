...
// Constructor
Map::Map() {
	addNode(0,0,stackSize()+1,false,false,false,false);
...
Map::Map(const double x, const double y, const bool north, const bool east,
	...
	addNode(x,y,stackSize()+1,north,east,south,west);
...
shared_ptr<Node> Map::getNode(const double x, const double y) {
	...
	for(auto& v: _vt.at(_hash(x,y))) {
		if(SAME_NODE(v->location.x, v->location.y, x, y)) {
			return v;
		}
	}
...
bool Map::addNode(const double x, const double y, const int stackSize, const bool north,
	...
	if(_vt.count(_hash(x,y)) == 0) {
		...
		auto cmp = [x,y](shared_ptr<Node>& n)->bool {
			return SAME_NODE(n->location.x, n->location.y, x, y);
		};
...
void Map::removeNode(shared_ptr<Node>& n) {
	...
	for(auto it = _vt.at(_hash(n->location.x,n->location.y)).begin(); it !=
		_vt.at(_hash(n->location.x,n->location.y)).end(); it++) {
		if(*it == n) {
			_vt.at(_hash(n->location.x,n->location.y)).erase(it);
			break;
		}
	}
